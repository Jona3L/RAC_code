...
class SingleThreadPrompter:
    ...
    def __init__(
        ...
        max_tokens: int = 1000, 
        llm_source: str = "meta-llama/Meta-Llama-3.1-70B-Instruct",
    ):
        self.env = env 
        ...
        login(hf_token)
        self.tokenizer = AutoTokenizer.from_pretrained(llm_source)
        self.model = AutoModelForCausalLM.from_pretrained(llm_source, torch_dtype=torch.bfloat16, device_map="auto")
        self.pipe = pipeline(
            "text-generation",
            model=self.model,
    ...
    def prompt_one_round(self, obs: EnvState, save_path: str = ""): 
        ...
        for i in range(self.num_replans): 
            ...
            response, usage = self.query_once(system_prompt, user_prompt="") # NOTE: single_thread doesn't use user role
    ...
    def query_once(self, system_prompt, user_prompt=""):
        ...
        for n in range(self.max_api_queries):
            ...
            try:
                ...
                response = outputs[0]['generated_text'].split(user_prompt)[-1].strip()
                ...
                print(f"Model error: {e}, trying again")
            continue
